...
def func(intt, t):
    ...
    d1 = -(m1+m2+m3)*gg*L1*sin(intt[0]) -(m2+m3)*L1*L2*sin(dif12)*intt[3]*intt[3] -m3*L1*L3*sin(dif13)*intt[5]*intt[5]
    d2 = -(m2+m3)*gg*L2*sin(intt[2]) + (m2+m3)*L1*L2*sin(dif12)*intt[1]*intt[1]   -m3*L2*L3*sin(dif23)*intt[5]*intt[5]
    d3 = -m3*gg*L3*sin(intt[4]) + m3*L1*L3*sin(dif13)*intt[1]*intt[1] + m3*L2*L3*sin(dif23)*intt[3]*intt[3]
...
fig = plt.figure()  
ax = fig.add_subplot(111, autoscale_on=False, xlim=(-1.2*(L1+L2+L3), 1.2*(L1+L2+L3)), ylim=(-1.2*(L1+L2+L3), 1.2*(L1+L2+L3)))
#ax.grid(linewidth=1)
